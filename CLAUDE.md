...
**Inversiones — alertas push reales (2026-09-01):** precios de cripto vía CoinGecko (público, sin clave) y de acciones vía Finnhub (clave gratis del usuario, `finnhub_api_key`). Las alertas de precio que el usuario define en la app se sincronizan a `alertas.json` en la raíz del repo vía la API de GitHub (Contents API), usando un token fine-grained del usuario (`github_pat`, permiso solo Contents R/W sobre este repo). Un workflow de **GitHub Actions** (`.github/workflows/alertas-precio.yml`, cron cada 15 min) corre `.github/scripts/check-alertas.js`, compara contra `alertas.json`, y manda el aviso por **Telegram** (bot "Jarvis" del proyecto de Finanzas — token en el secret `TELEGRAM_BOT_TOKEN` del repo, no en el código). El estado de qué ya se avisó vive en `estado.json` (el propio workflow lo commitea). Requiere 3 GitHub Actions secrets configurados una vez en Settings → Secrets: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `FINNHUB_API_KEY`.
